function layDanhSachChuDe() {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost:8089/api/chude",
        dataType: "json",
...
function xuLyThemMoi() {
    ...
    var urlPost = 'http://localhost:8089/api/chude/add';
    var methodType = "POST";
    alert(ma);
    //TH sửa
    if (ma.length > 0) {
        alert(ma);
        urlPost = 'http://localhost:8089/api/chude/' + ma;
        methodType = "PUT";
    }
...
function thongTinChiTiet(maCD) {
    $.ajax({
        type: "GET",
        url: "http://localhost:8089/api/chude/" + maCD,
        data: {
            id: maCD
        },
        dataType: 'json',
