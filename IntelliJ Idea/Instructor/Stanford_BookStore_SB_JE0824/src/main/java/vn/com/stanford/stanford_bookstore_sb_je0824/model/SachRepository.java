...
public interface SachRepository extends JpaRepository<Sach, String> {
    @Query("SELECT s FROM Sach s WHERE ((s.tenSach like :tuKhoa OR s.maSach = :tuKhoa OR s.tacGia like :tuKhoa) AND (LENGTH(s.maChuDe) = 0 OR (LENGTH(s.maChuDe) > 0 AND s.maChuDe = :maChuDe)))")
